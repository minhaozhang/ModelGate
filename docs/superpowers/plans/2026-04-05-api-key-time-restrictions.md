...
**Files:**
- Create: `app/core/database.py` (add `ApiKeyTimeRule` class)
...
Run: `python -m app.main` and check logs for no errors.

- [x] **Step 4: Commit**

## Task 2: Update `api_keys_cache` to Include Time Rules
**Files:**
- Modify: `app/services/auth.py` (load_api_keys)
- Modify: `app/core/config.py` (cache type hint)
...
**Files:**
- Modify: `app/services/auth.py` (validate_api_key)
...
**Files:**
- Create: `app/routes/time_rules.py`
- Modify: `app/main.py` (register router)
- [ ] **Step 1: Create app/routes/time_rules.py with endpoints**
...
All endpoints require admin auth (`require_admin`).
 ...
 Same patterns as `app/routes/keys.py`.
- [ ] **Step 2: Register router in app/main.py**
- [ ] **Step 3: Commit**

## Task 5: Update Admin Frontend for Time Rules UI
**Files:**
- Modify: admin template to `web/templates/admin/` (add time rules management section to API key edit page)
...
Add a section with fields for rule name, time range (start time, end time), date range (start date, end date), active toggle. Use the same modal/dialog pattern as existing key edit forms.
 ...
 Add a confirmation dialog on `web/templates/admin/base.html` or similar base template for deletion confirmation.
...
**Files:**
- Modify: admin template in `web/templates/admin/` (update i18n)
...
Similar to `keys` in `app/routes/keys.py` — add `time_rules` to the response dict.
